    ...
    let ctx = gsap.context(()=>{
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub: 4,  // tempo per arrivare in posizione ( 4 secondi )
        }
      });


      tl.fromTo(".tech-row", {
        x: (index)=>{  // In questo modo non si alliniano mai precisi con il random
  ...
  return (
    ...
    >
      ...
      {slice.primary.techs.map(({tech_color, tech_name}, index)=>(
        <div key={index} className="tech-row mb-8 flex items-center justify-center gap-4 text-slate-700" area-label={tech_name || undefined}>
